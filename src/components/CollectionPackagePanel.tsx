...
export function CollectionPackagePanel({
  ...
  async function buy() {
    ...
    setMsg(null);
    try {
      // MVP: simulate package prepare (same-network). Wallet pay UX can resume per purchase.
      const res = await fetch(`/api/collections/${collectionId}/package`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          payNetwork: data?.network || "ethereum",
      ...
      setMsg(
        `Package prepared (${body.purchaseIds?.length ?? 0} works, $${body.amountUsd}). Same-network only.`,
      );
      load();
      router.refresh();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: "0 0 0.75rem", color: "var(--ink-muted)", fontSize: "0.9rem" }}>
        ...
        Same-network checkout only for MVP.
      </p>
      {isOwner ? (
